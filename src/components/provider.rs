            ...
            >
                ...
                <div class="drawer-content scroll-smooth">
                    <div class="navbar bg-opacity-90 backdrop-blur bg-base-200 shadow-sm top-0 sticky z-30">
                        ...
                        <div class="navbar-center"> <a href="#landing"> {"RizalAchp"}</a> </div>
                ...
                <ul class="menu p-4 overflow-y-auto w-80 bg-base-100 backdrop-blur transition-all bg-opacity-90">
                    <li><a onclick={route_to(&link, PageRoute::Projects)} href="">{"Projects"}</a></li>
                    <li><a onclick={route_to(&link, PageRoute::Blog)} href="">{"Blog"}</a></li>
                    <li><a onclick={route_to(&link,PageRoute::AboutMe) } href="">{"About"}</a> </li>
                </ul>
                </div>
            </div>
        )
...
#[inline]
pub fn route_to<COMP: Component, <COMP as Component>::Message: From<Msg>>(link: &Scope<COMP>, rt: PageRoute) -> Callback<MouseEvent> {
    if let Some(parr) = link.get_parent().cloned() {
        parr.downcast::<crate::App>().send_message(OnRouting(rt))
    }
    link.callback(move |_: MouseEvent| Msg::ToggleNav)
}
